...
bool findCamera (char * deviceStr) {
	FILE *fp;
	bool found = false;

	fp = fopen ("/dev/video0","r" );
	if ( fp) {
		fclose(fp);
		strcpy( deviceStr, "device=/dev/video0");
		found = true;
	}
	else {
		fp = fopen("/dev/video1", "r");
		if (fp) {
			fclose(fp);
			strcpy(deviceStr, "device=/dev/video1");
			found = true;
		}
	}
	return found;
...
bool setVideoTask(streamerTask_t task, int UDPport, char *pText) {
	...
	case VIDEOTASK_STREAM:
		if (actualTask != VIDEOTASK_STREAM) { // do nothing if already showing
			...
			if (findCamera( deviceStr)) {
				sprintf(pipelinedescriptor,	"gst-launch-1.0 v4l2src %s ! videoconvert !"
						" videoscale ! video/x-raw,width=800,height=600 ! jpegenc ! rtpjpegpay ! udpsink", deviceStr);
			}
			else
	...
	case VIDEOTASK_SHOWCAMERA:
		if (actualTask != VIDEOTASK_SHOWCAMERA) {
			...
			if (findCamera( deviceStr)) {
					sprintf(pipelinedescriptor,	"gst-launch-1.0 v4l2src %s ! videoconvert ! videoscale ! video/x-raw,width=800,height=600"
							" ! videoflip method=3 ! nxvideosink", deviceStr);
			}
			else
				sprintf(pipelinedescriptor,
								"gst-launch-1.0 videotestsrc pattern=spokes is-live=true ! "
								"videoconvert ! videoscale method=1"
								" ! video/x-raw,width=800,height=600,framerate=30/1 ! "
								"videoflip method=3 ! nxvideosink",
								NULL);
